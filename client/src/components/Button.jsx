...
export default function Button({ children, variant='primary', size='md', to, className='', type='button', onClick, disabled=false, ariaLabel }){
  ...
  const variants = {
    primary: 'text-white bg-gradient-to-r from-[rgba(var(--brand),1)] to-[rgba(6,182,212,0.12)] shadow-[0_8px_30px_rgba(var(--brand),0.12)]',
  ...
  const base = 'inline-flex items-center justify-center gap-2 font-semibold rounded-lg transform transition duration-180 ease-out select-none';
  const hover = disabled ? '' : 'hover:-translate-y-1 hover:shadow-glow-md';
  const focus = 'focus:outline-none focus:ring-4 focus:ring-[rgba(var(--brand),0.08)]';
